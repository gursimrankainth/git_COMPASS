...
for run in "${!run_files[@]}"; do
  files=(${run_files[$run]})
  total_files=${#files[@]}
  echo "Processing run $run with $total_files files..."
  ...
  for ((idx=0; idx<total_files; idx++)); do
    ...
    if [[ -n "$match_line" ]]; then
      ...
      for ((i=0; i<num_patterns; i++)); do
        read -r n1 n2 <<< "${nums[$i]}"
        # Safely add numbers as integers (assuming all numbers are integers)
  ...
  echo -e "\nFinished processing run $run."

  # Write output file
  out_file="$out_dir/tree-$run.txt"
  {
    for ((i=0; i<num_patterns; i++)); do
